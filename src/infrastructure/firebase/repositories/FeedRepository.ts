...
export class FeedRepository {
  ...
  async getFeedForFollowing(
    ...
    try {
      ...
      for (const batchIds of batches) {
        ...
        if (cursor) {
          const cursorDoc = await getDoc(firestoreDoc(db, this.collectionName, cursor))
          if (cursorDoc.exists()) {
      ...
      const sessions = await this.mapper.toDomainListEnriched(limitedDocs.slice(0, limit))
   ...
   */
  async getPublicFeed(limit: number = 20, cursor?: string): Promise<FeedResult> {
    try {
      let q = query(
        collection(db, this.collectionName),
      ...
      const sessions = await this.mapper.toDomainListEnriched(snapshot.docs.slice(0, limit))
