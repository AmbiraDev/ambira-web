...
function Sidebar({ type, children, className = '' }: SidebarProps) {
  return (
    <aside
      className={`hidden lg:block ${type === 'left' ? 'lg:w-80' : 'lg:w-72'} ${className}`}
    >
    ...
    <Sidebar type="right">
      <div className="space-y-8">
        ...
        <div>
          ...
          <button className="text-sm text-[#0066CC] hover:underline transition-colors px-1 font-medium">
            View All Challenges →
          </button>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {[1, 2, 3].map(i => (
              <div
                key={i}
                className="flex items-center space-x-3 px-1 py-2 hover:bg-gray-50 rounded-lg transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-600 truncate">
                    Fan favorite on Ambira
                  </p>
                </div>
                <button className="px-3 py-1.5 text-xs bg-[#0066CC] text-white rounded hover:bg-[#0066DD] transition-colors font-medium flex-shrink-0">
                  Follow
                </button>
              </div>
            ))}
          </div>
          <button className="mt-4 text-sm text-[#0066CC] hover:underline transition-colors px-1 font-medium">
