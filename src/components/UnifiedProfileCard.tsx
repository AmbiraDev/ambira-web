    ...
    <div className={`bg-white rounded-xl border border-gray-200 p-6 ${className}`}>
      ...
      {profilePicture ? (
        ...
        <div className="w-32 h-32 min-w-[8rem] aspect-square bg-gradient-to-br from-[#FC4C02] to-[#FF8800] rounded-full flex items-center justify-center mb-5 shadow-md flex-shrink-0">
          <span className="text-white font-bold text-5xl">{name.charAt(0).toUpperCase()}</span>
        </div>
      )}
      ...
      <div className="mb-1">
        ...
        {pronouns && <p className="text-gray-400 text-sm mt-0.5">{pronouns}</p>}
      </div>
      ...
      {/* Tagline */}
      {tagline && <p className="text-gray-600 text-sm mb-3 font-medium">{tagline}</p>}

      {/* Bio */}
      {showBio && bio && <p className="text-gray-700 mb-3 leading-relaxed">{bio}</p>}
