...
describe('Image Upload Integration Flow - Simplified', () => {
  ...
  describe('Feed display integration', () => {
    ...
    it('should handle sessions without images in feed', () => {
      ...
      const sessionsWithImages = sessions.filter(
        s => s.images && s.images.length > 0
      );
      const sessionsWithoutImages = sessions.filter(
        s => !s.images || s.images.length === 0
      );
      ...
      const uploadedImages = [
        {
          ...
          path: 'session-images/user/img1.jpg',
        },
        {
          url: 'https://example.com/img2.jpg',
          path: 'session-images/user/img2.jpg',
        },
      ];
  ...
  describe('Error handling', () => {
    ...
    it('should allow partial upload success', () => {
      ...
      const uploadResults = Array.from(
        { length: successfulUploads },
        (_, i) => ({
