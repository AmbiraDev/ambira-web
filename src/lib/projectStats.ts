...
export function calculateProjectStats(
  ...
  const totalProgressPercentage = totalTarget ? Math.min(100, (totalHours / totalTarget) * 100) : 0

  // Calculate average session duration (in minutes)
  const averageSessionDuration = sessionCount > 0 ? (totalHours * 60) / sessionCount : 0
 ...
 */
export function calculateWeeklyAverage(sessions: Session[], weeks: number = 4): number {
  const cutoffDate = new Date()
  cutoffDate.setDate(cutoffDate.getDate() - weeks * 7)

  const recentSessions = sessions.filter((session) => new Date(session.createdAt) >= cutoffDate)
