...
const _processPosts = async (postDocs: DocumentSnapshot[]): Promise<PostWithDetails[]> => {
  ...
  for (let i = 0; i < postDocs.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (postDoc) => {
      ...
      // Get session data
      const sessionDoc = await getDoc(doc(db, 'sessions', postData.sessionId as string))
      ...
      if (projectId) {
        try {
          const projectDoc = await getDoc(
            doc(db, 'projects', postData.userId as string, 'userProjects', projectId as string)
          )
      ...
      const supportDoc = auth.currentUser
        ? await getDoc(doc(db, 'postSupports', `${auth.currentUser.uid}_${postDoc.id}`))
        : null
...
export const firebasePostApi = {
  ...
  listenToSessionUpdates: (
    sessionIds: string[],
    callback: (updates: Record<string, { supportCount: number; isSupported: boolean }>) => void
  ) => {
