...
export async function getActivityPreference(
  ...
  try {
    ...
    const prefDoc = await getDoc(doc(db, `users/${userId}/activityPreferences/${typeId}`))
...
export async function getRecentActivitiesWithDetails(
  ...
  try {
    ...
    return activities.filter((activity): activity is ActivityType => activity !== null)
  } catch (_error) {
 ...
 * });
 */
export async function getAllActivitiesWithUsage(userId: string): Promise<ActivityTypeWithUsage[]> {
  try {
    if (!userId) {
      throw new Error('User ID is required')
    }
    ...
    const activitiesWithUsage: ActivityTypeWithUsage[] = allActivities.map((activity) => {
 ...
 */
export async function getActivitiesWithSessions(userId: string): Promise<ActivityTypeWithUsage[]> {
  try {
    if (!userId) {
      throw new Error('User ID is required')
    }
    ...
    const validActivities = activities.filter((a): a is ActivityTypeWithUsage => a !== null)
...
export async function getActivityUsageStats(userId: string): Promise<{
  ...
  try {
    ...
    const totalSessions = preferences.reduce((sum, pref) => sum + pref.useCount, 0)
    const averageSessionsPerActivity = activitiesUsed > 0 ? totalSessions / activitiesUsed : 0
