...
export const firebaseGroupApi = {
  ...
  getGroup: async (groupId: string): Promise<Group | null> => {
    try {
      ...
      throw new Error(typeof _error === 'string' ? _error : 'Failed to get group')
    }
  },
  ...
  createGroup: async (groupData: CreateGroupData, userId: string): Promise<Group> => {
    try {
      ...
      const newGroup = {
        ...removeUndefinedFields(groupData as unknown as Record<string, unknown>),
        adminUserIds: [userId],
        memberIds: [userId],
        memberCount: 1,
   ...
   * Update a group
   */
  updateGroup: async (groupId: string, updateData: UpdateGroupData): Promise<void> => {
    try {
      const docRef = doc(db, 'groups', groupId)
      const updatePayload = {
        ...removeUndefinedFields(updateData as unknown as Record<string, unknown>),
  ...
  getUserGroups: async (userId: string, limit?: number): Promise<Group[]> => {
    try {
      ...
      return domainGroups.map((g) => ({
        ...
        privacySetting: g.privacy === 'approval-required' ? 'approval-required' : 'public',
        location: g.location,
