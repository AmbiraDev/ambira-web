    ...
    try {
      ...
      if (userData.currentStreak >= 7 && !unlockedTypes.has('streak-7')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'streak-7', sessionId)
        )
      }
      if (userData.currentStreak >= 30 && !unlockedTypes.has('streak-30')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'streak-30', sessionId)
        )
      }
      if (userData.currentStreak >= 100 && !unlockedTypes.has('streak-100')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'streak-100', sessionId)
        )
      }
      if (userData.currentStreak >= 365 && !unlockedTypes.has('streak-365')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'streak-365', sessionId)
        )
      }
      ...
      if (userData.totalHours >= 10 && !unlockedTypes.has('hours-10')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'hours-10', sessionId)
        )
      }
      if (userData.totalHours >= 50 && !unlockedTypes.has('hours-50')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'hours-50', sessionId)
        )
      }
      if (userData.totalHours >= 100 && !unlockedTypes.has('hours-100')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'hours-100', sessionId)
        )
      }
      if (userData.totalHours >= 500 && !unlockedTypes.has('hours-500')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'hours-500', sessionId)
        )
      }
      if (userData.totalHours >= 1000 && !unlockedTypes.has('hours-1000')) {
        newAchievements.push(
          await firebaseAchievementApi.awardAchievement(userId, 'hours-1000', sessionId)
        )
      }
      ...
      if (userData.recentSession) {
        ...
        if (sessionHour < 6 && !unlockedTypes.has('early-bird')) {
          newAchievements.push(
            await firebaseAchievementApi.awardAchievement(userId, 'early-bird', sessionId)
          )
        }

        if (sessionHour >= 22 && !unlockedTypes.has('night-owl')) {
          newAchievements.push(
            await firebaseAchievementApi.awardAchievement(userId, 'night-owl', sessionId)
          )
        }
      }
