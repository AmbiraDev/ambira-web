...
export const ACTIVITY_TYPE_KEYS = {
  ...
  custom: (userId: string) =>
    [...ACTIVITY_TYPE_KEYS.all(), 'custom', userId] as const,
  combined: (userId: string) =>
    [...ACTIVITY_TYPE_KEYS.all(), 'combined', userId] as const,
};
 ...
 */
export function useSystemActivityTypes(
...
export function useCreateCustomActivity(
  ...
  >({
    ...
    onMutate: async newActivity => {
      if (!user)
        return { previousCustomTypes: null, previousCombinedTypes: null };
      ...
      const previousCustomTypes = queryClient.getQueryData(
        ACTIVITY_TYPE_KEYS.custom(user.id)
      );
      const previousCombinedTypes = queryClient.getQueryData(
...
export function useDeleteCustomActivity(
  options?: Partial<
    UseMutationOptions<void, Error, string, DeleteCustomActivityContext>
  >
) {
  const queryClient = useQueryClient();
  const { user } = useAuth();
