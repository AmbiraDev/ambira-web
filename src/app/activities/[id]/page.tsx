...
function ProjectDetailContent({ projectId }: { projectId: string }) {
  ...
  const calculatedStats = React.useMemo(() => {
    ...
    const totalHours = periodSessions.reduce((sum, s) => sum + s.duration / 3600, 0);
    const sessionCount = periodSessions.length;
    const avgDuration = sessionCount > 0
      ? periodSessions.reduce((sum, s) => sum + s.duration, 0) / sessionCount / 60
      : 0;
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            {chartType === 'bar' ? (
                              <BarChart data={chartData} margin={{ top: 5, right: 5, left: -30, bottom: 0 }}>
                                ...
                                <Tooltip content={<CustomTooltip />} />
                                <Bar dataKey="avgDuration" fill="#00BA7C" radius={[4, 4, 0, 0]} name="Minutes" />
                              </BarChart>
                            ) : (
                ...
                <div className="max-w-3xl mx-auto">
                  {isLoadingSessions ? (
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                      ...
                      <p className="text-gray-600">Start tracking time on this activity to see sessions here.</p>
                    </div>
                  ) : (
