...
import { validateOrThrow, CreateGroupSchema, GroupMembershipSchema } from '@/lib/validation'
...
export class GroupService {
  ...
  async createGroup(data: unknown, userId: string): Promise<Group> {
    ...
    const categoryForDomain = validated.category === 'other' ? 'learning' : validated.category

    // Validate category matches domain model
    const validCategories = ['work', 'study', 'side-project', 'learning'] as const
    if (!validCategories.includes(categoryForDomain as (typeof validCategories)[number])) {
      throw new Error(
