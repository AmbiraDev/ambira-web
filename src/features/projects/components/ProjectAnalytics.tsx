...
export const ProjectAnalytics: React.FC<ProjectAnalyticsProps> = ({ projectId, projectName }) => {
  const { user } = useAuth()
  const defaultPeriod: AnalyticsPeriod = PERIODS[1] ||
    PERIODS[0] || { label: '7D', value: '7d' as const, days: 7 }
  const [selectedPeriod, setSelectedPeriod] = useState<AnalyticsPeriod>(defaultPeriod)
    ...
    try {
      ...
      setAnalyticsData({
        ...
        averageSessionDuration:
          sessions.length > 0
            ? Math.round(
                (sessions.reduce((sum, s) => sum + s.duration, 0) / sessions.length / 60) * 10
              ) / 10
            : 0,
        goalProgress,
        cumulativeHours,
  ...
  const daysUntilGoal = analyticsData.goalProgress.estimatedCompletion
    ? Math.ceil(
        (analyticsData.goalProgress.estimatedCompletion.getTime() - Date.now()) /
          (1000 * 60 * 60 * 24)
      )
    : null
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-xs text-gray-500 mt-1">{analyticsData.weeklyAverage}h per week avg</p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {daysUntilGoal && daysUntilGoal > 0 && (
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">{daysUntilGoal} days</p>
                    <p className="text-xs text-gray-500 mt-1">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {Math.max(
                      0,
                      analyticsData.goalProgress.target - analyticsData.goalProgress.current
                    )}
                    h
                  </p>
                </div>
            ...
            <p className="text-sm text-gray-700">
              ...
              <span className="font-semibold">
                {analyticsData.sessionFrequency.length > 0
                  ? (
                      analyticsData.sessionFrequency.reduce((sum, d) => sum + d.value, 0) /
