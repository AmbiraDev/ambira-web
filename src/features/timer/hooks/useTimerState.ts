 ...
 */
export function useTimerState({ activeSession, currentProject, onAutoSave }: UseTimerStateOptions) {
  ...
  useEffect(() => {
    ...
    if (isPaused) {
      ...
      elapsed = Math.floor((now.getTime() - activeSession.startTime.getTime()) / 1000)
    }
