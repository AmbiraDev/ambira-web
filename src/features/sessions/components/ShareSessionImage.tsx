...
export const ShareSessionImage: React.FC<ShareSessionImageProps> = ({
  ...
  const content = (
    <>
        ...
        >
          ...
          <div className="px-8 py-6">
            ...
            {session.images && session.images.length > 0 && (
              <div
                className={`mb-4 ${session.images.length === 1 ? '' : session.images.length === 2 ? 'grid grid-cols-2 gap-2' : 'grid grid-cols-3 gap-2'}`}
              >
      ...
      {exportError && (
        <div
          className={`${isPage ? 'px-4 md:px-0' : 'mx-6'} mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm`}
        >
          {exportError}
        </div>
      )}
      ...
      {/* Actions */}
      <div
        className={`flex items-center justify-end gap-3 ${isPage ? 'px-4 md:px-0 py-6' : 'p-4 border-t border-gray-200'}`}
      >
        <button
          onClick={onClose}
