...
export class ProfileStatsCalculator {
   ...
   */
  private calculateDailyData(sessions: Session[], days: number, now: Date): ChartDataPoint[] {
   ...
   */
  private calculateWeeklyData(sessions: Session[], weeks: number, now: Date): ChartDataPoint[] {
    const data: ChartDataPoint[] = []
   ...
   */
  private calculateMonthlyData(sessions: Session[], months: number, now: Date): ChartDataPoint[] {
    const monthNames = [
      'Jan',
      'Feb',
      'Mar',
  ...
  filterSessionsByActivity(sessions: Session[], activityId: string): Session[] {
    ...
    return sessions.filter(
      (s) =>
        s.activityId === activityId || s.projectId === activityId || s.groupIds.includes(activityId)
    )
  }
