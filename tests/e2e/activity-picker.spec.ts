...
test.describe('Activity Picker Journey', () => {
  ...
  test.describe('Activity Picker Opening', () => {
    ...
    test('should show activity options after opening picker', async ({ page }) => {
  ...
  test.describe('Recent Activities Display', () => {
    test('should display recent activities in horizontal bar', async ({ page }) => {
    ...
    test('should allow quick selection from recent activities', async ({ page }) => {
      // Open picker
      const pickerButton = page.locator('button:has-text("Activity")').first()
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        const firstActivity = page
          .locator('[class*="activity"] button, [role="option"], button[class*="item"]')
          .first()
        ...
        if ((await firstActivity.count()) > 0) {
          ...
          const selectedText = await page.textContent('[class*="selected"], [class*="active"]')
  ...
  test.describe('All Activities List', () => {
    test('should display all activities when scrolling down', async ({ page }) => {
      // Open picker
      const pickerButton = page.locator('button:has-text("Activity")').first()
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        if ((await listContainer.count()) > 0) {
          ...
          const initialOptions = page.locator('[role="option"], [class*="activity-item"]')
    ...
    test('should display default system activities', async ({ page }) => {
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        const pageText = await page.textContent('body')
        const defaultActivities = ['Work', 'Coding', 'Reading', 'Learning', 'Study']

        const hasDefaults = defaultActivities.some((activity) => pageText?.includes(activity))

        expect(hasDefaults).toBeTruthy()
      }
    })

    test('should display custom activities created by user', async ({ page }) => {
      // Open picker
      const pickerButton = page.locator('button:has-text("Activity")').first()
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        const allActivities = page.locator('[role="option"], [class*="activity-item"]')
  ...
  test.describe('Activity Selection', () => {
    ...
    test('should update selected activity display', async ({ page }) => {
      ...
      if ((await initialButton.count()) > 0) {
        ...
        const secondActivity = page.locator('[role="option"], [class*="activity-item"]').nth(1)
        ...
        if ((await secondActivity.count()) > 0) {
          ...
          const updatedButton = page.locator('button:has-text("Activity")').first()
  ...
  test.describe('Create Activity from Picker', () => {
    ...
    test('should show newly created activity in picker immediately', async ({ page }) => {
        ...
        if ((await createButton.count()) > 0) {
          ...
          if ((await nameInput.count()) > 0) {
            ...
            if ((await submitBtn.count()) > 0) {
              ...
              const finalOptions = page.locator('[role="option"], [class*="activity-item"]')
  ...
  test.describe('Responsive Design', () => {
    ...
    test('should be scrollable on small screens', async ({ page }) => {
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        const list = page.locator('[role="listbox"], [class*="dropdown"]').first()
  ...
  test.describe('Accessibility', () => {
    test('should pass accessibility audit on activity picker', async ({ page, makeAxeBuilder }) => {
    ...
    test('should support keyboard navigation through activities', async ({ page }) => {
      // Open picker
      const pickerButton = page.locator('button:has-text("Activity")').first()
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        const focusedElement = await page.evaluate(() => {
          return document.activeElement?.getAttribute('role') || document.activeElement?.tagName
        })
    ...
    test('should have proper ARIA labels on activity items', async ({ page }) => {
      // Open picker
      const pickerButton = page.locator('button:has-text("Activity")').first()
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        if (count > 0) {
          ...
          const hasAriaLabel = await activities.first().getAttribute('aria-label')
    ...
    test('should announce activity selection to screen readers', async ({ page }) => {
      // Open picker
      const pickerButton = page.locator('button:has-text("Activity")').first()
  ...
  test.describe('Visual Regression', () => {
    test('should have consistent picker layout on desktop', async ({ page }) => {
      await page.setViewportSize({ width: 1440, height: 900 })
      await page.goto('/timer')
      await page.waitForLoadState('networkidle')
  ...
  test.describe('No Console Errors', () => {
    test('should not have critical console errors during picker interaction', async ({ page }) => {
      const errors: string[] = []
