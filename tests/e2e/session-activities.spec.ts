...
test.describe('Session with Activities Journey', () => {
  ...
  test.describe('Select Activity and Start Timer', () => {
    test('should allow selecting activity before starting timer', async ({
      page,
    }) => {
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        if ((await activity.count()) > 0) {
          ...
          const icons = page.locator(
            'svg, [class*="icon"], img[alt*="activity"]'
          );
    ...
    test('should prevent starting timer without activity selected', async ({
      page,
    }) => {
      ...
      if ((await startButton.count()) > 0) {
        ...
        expect(
          isDisabled || (hasActivity && hasActivity.length > 0)
        ).toBeTruthy();
      }
    });
  });
  ...
  test.describe('Save Session with Activity', () => {
    ...
    test('should allow editing session details before saving', async ({
      page,
    }) => {
      // Start session flow
      const pickerButton = page.locator('button:has-text("Activity")').first();
    ...
    test('should require activity before saving session', async ({ page }) => {
      ...
      const saveButton = page
        .locator(
          'button:has-text("Save"), button:has-text("Done"), button:has-text("Confirm")'
        )
        .first();
      ...
      if ((await saveButton.count()) > 0) {
        ...
        expect(
          isDisabled || (await page.textContent('body'))?.includes('Activity')
        ).toBeTruthy();
      }
    });
  });
  ...
  test.describe('Session Appears in Feed', () => {
    ...
    test('should show session with activity icon in feed', async ({ page }) => {
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        const activityIcons = page.locator(
          '[class*="activity-icon"], [class*="icon"], svg'
        );
  ...
  test.describe('Recent Activities Update', () => {
    test('should update recent activities list after logging session', async ({
      page,
    }) => {
      ...
      const initialActivities = page.locator(
    ...
    test('should show recently used activity first in picker', async ({
      page,
    }) => {
  ...
  test.describe('Activity Stats Page', () => {
    ...
    test('should update session count after logging new session', async ({
      page,
    }) => {
      // Get initial stats
      await page.goto('/settings/activities');
  ...
  test.describe('Accessibility', () => {
    ...
    test('should be keyboard accessible for session logging', async ({
      page,
    }) => {
  ...
  test.describe('No Console Errors', () => {
    test('should not have critical errors during session logging', async ({
      page,
    }) => {
      const errors: string[] = [];
