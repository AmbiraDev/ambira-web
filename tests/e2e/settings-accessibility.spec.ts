...
test.describe('Settings Page Accessibility', () => {
  ...
  test.describe('Keyboard Navigation', () => {
    ...
    test('should have logical tab order', async ({ page }) => {
      ...
      const focusableElements = await page.locator('button, input, select, textarea, a[href]')
  ...
  test.describe('Screen Reader Support', () => {
    ...
    test('should announce form errors to screen readers', async ({ page }) => {
      ...
      const errorRole = await nameInput.evaluate((el) => el.getAttribute('aria-invalid'))
    ...
    test('should indicate required fields', async ({ page }) => {
      ...
      const isRequired = await nameInput.evaluate((el) => el.getAttribute('required') !== null)
  ...
  test.describe('Form Accessibility', () => {
    test('should have form labels associated with inputs', async ({ page }) => {
      ...
      const nameLabel = page.locator('label').filter({ hasText: /Name/ }).first()
      const nameInput = page.getByLabel(/Name/i).first()
      ...
      const hasAriaLabel = await nameInput.evaluate((el) => el.getAttribute('aria-label') !== null)
      expect(hasAriaLabel).toBe(true)
    })

    test('should group related form fields', async ({ page }) => {
      // Social links should be grouped
      const socialLinksHeading = page.getByText('Links').or(page.getByText('Social Links'))
    ...
    test('should have input type attributes', async ({ page }) => {
      ...
      const type = await websiteInput.first().evaluate((el) => el.getAttribute('type'))
  ...
  test.describe('ARIA Attributes', () => {
    ...
    test('should indicate disabled buttons', async ({ page }) => {
      ...
      const disabled = await saveButton.evaluate((el) => el.getAttribute('disabled') !== null)
    ...
    test('should indicate live regions for status messages', async ({ page }) => {
    ...
    test('should have aria-describedby for help text', async ({ page }) => {
      ...
      const describedBy = await bioInput.evaluate((el) => el.getAttribute('aria-describedby'))
  ...
  test.describe('Mobile Accessibility', () => {
    test('should be touch-friendly on mobile', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(count, 3); i++) {
        ...
        expect((box?.height || 0) + (box?.width || 0) / 2).toBeGreaterThanOrEqual(44)
      }
    })
  ...
  test.describe('Semantic HTML', () => {
    ...
    test('should use semantic list elements where appropriate', async ({ page }) => {
