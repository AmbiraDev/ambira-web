...
test.describe('Notifications UI', () => {
  ...
  test.describe('Notification panel display', () => {
    ...
    test('should open notification panel when icon is clicked', async ({
      page,
    }) => {
    ...
    test('should display empty state when no notifications', async ({
      ...
      const hasNotifications =
        (await page.locator('[data-notification-id]').count()) > 0;
  ...
  test.describe('Mark all as read functionality', () => {
    ...
    test('should mark all notifications as read when button is clicked', async ({
      page,
    }) => {
      // Open notification panel
      const notificationIcon = page
      ...
      if (await markAllButton.isVisible()) {
        ...
        const unreadBadgesBeforeCount = await page
          .locator('.bg-\\[\\#0066CC\\]')
          .count();
        ...
        const unreadBadgesAfterCount = await page
          .locator('.bg-\\[\\#0066CC\\]')
          .count();
        expect(unreadBadgesAfterCount).toBeLessThanOrEqual(
          unreadBadgesBeforeCount
        );
      }
    });

    test('should update unread count badge after marking all as read', async ({
      page,
    }) => {
      // Open notification panel
      const notificationIcon = page
      ...
      const unreadBadgeBefore = notificationIcon
        .first()
        .locator('.bg-\\[\\#0066CC\\]');
      const hadUnreadBefore = await unreadBadgeBefore
        .isVisible()
        .catch(() => false);
      ...
      if (await markAllButton.isVisible()) {
        ...
        const unreadBadgeAfter = notificationIcon
          .first()
          .locator('.bg-\\[\\#0066CC\\]');

        // Badge should be hidden or count reduced
        if (hadUnreadBefore) {
          const hasUnreadAfter = await unreadBadgeAfter
            .isVisible()
            .catch(() => false);
  ...
  test.describe('Clear all functionality', () => {
    ...
    test('should clear all notifications when button is clicked', async ({
      page,
    }) => {
      // Open notification panel
      const notificationIcon = page
    ...
    test('should hide clear all button after clearing all notifications', async ({
      page,
    }) => {
      // Open notification panel
      const notificationIcon = page
  ...
  test.describe('Individual notification actions', () => {
    test('should show delete button on hover', async ({ page }) => {
      ...
      if (await firstNotification.isVisible()) {
        ...
        const deleteButton = firstNotification
    ...
    test('should delete individual notification when delete button is clicked', async ({
      ...
      if (await firstNotification.isVisible()) {
        ...
        const notificationId = await firstNotification.getAttribute(
  ...
  test.describe('Notification types and icons', () => {
    test('should display correct icon for different notification types', async ({
      page,
    }) => {
      // Open notification panel
      const notificationIcon = page
    ...
    test('should display relative time for notifications', async ({ page }) => {
      ...
      if (await firstNotification.isVisible()) {
        ...
        const timeIndicator = firstNotification.locator(
