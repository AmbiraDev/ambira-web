...
test.describe('Activities Accessibility Tests', () => {
    ...
    test('should have form labels for all inputs', async ({ page }) => {
      ...
      if ((await createButton.count()) > 0) {
        ...
        for (let i = 0; i < inputCount; i++) {
          ...
          expect(
            ariaLabel || ariaLabelledby || placeholder || hasAssociatedLabel
          ).toBeTruthy();
        }
      }
    });

    test('should have proper ARIA roles on custom elements', async ({
      page,
    }) => {
      await page.goto('/settings/activities');
      await page.waitForLoadState('networkidle');
  ...
  test.describe('Activity Picker Accessibility', () => {
    test('should pass WCAG 2.1 AA audit on activity picker', async ({
      page,
      makeAxeBuilder,
    }) => {
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        if (violations.length > 0) {
          console.log(
            'Activity Picker WCAG violations:\n',
            formatA11yViolations(violations)
          );
        }
    ...
    test('should announce selected activity to screen readers', async ({
      page,
    }) => {
      await page.goto('/timer');
      await page.waitForLoadState('networkidle');
      ...
      if ((await pickerButton.count()) > 0) {
        ...
        expect(
          (await liveRegions.count()) + (await statusRole.count())
  ...
  test.describe('Keyboard Navigation', () => {
    test('should allow tabbing through all interactive elements', async ({
      page,
    }) => {
      await page.goto('/settings/activities');
      await page.waitForLoadState('networkidle');
    ...
    test('should have visible focus indicators', async ({ page }) => {
      ...
      if ((await button.count()) > 0) {
        ...
        expect(
          focusStyle.hasOutline ||
            focusStyle.hasBoxShadow ||
            focusStyle.hasFocusVisible
        ).toBeTruthy();
      }
    });
  });
  ...
  test.describe('Form Accessibility', () => {
    ...
    test('should provide validation feedback in real-time', async ({
      page,
    }) => {
      await page.goto('/settings/activities');
      await page.waitForLoadState('networkidle');
  ...
  test.describe('Focus Management', () => {
    ...
    test('should set focus to first element in modal', async ({ page }) => {
      ...
      if ((await createButton.count()) > 0) {
        ...
        expect(
          focused === 'INPUT' || focused === 'BUTTON' || focused === 'TEXTAREA'
        ).toBeTruthy();
      }
    });
  });
  ...
  test.describe('No Console Errors During Accessibility', () => {
    test('should not have errors when testing accessibility', async ({
      page,
    }) => {
      const errors: string[] = [];
