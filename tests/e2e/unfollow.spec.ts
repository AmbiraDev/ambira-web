...
test.describe('Unfollow Functionality', () => {
  ...
  test.describe('Unfollow from Profile Header', () => {
    test('should successfully unfollow a user from their profile page', async ({
      page,
    }) => {
      ...
      const usernameText = await userCard
        .locator('text=/^@[a-zA-Z0-9_]+$/')
        .first()
        .textContent();
      ...
      const initialFollowerText =
        (await followerCountElement.textContent()) || '0';
      const initialFollowerCount = parseInt(
        initialFollowerText.match(/\d+/)?.[0] || '0'
      );
      ...
      const updatedFollowerText =
        (await followerCountElement.textContent()) || '0';
      const updatedFollowerCount = parseInt(
        updatedFollowerText.match(/\d+/)?.[0] || '0'
      );
    ...
    test('should show loading state during unfollow operation', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
  ...
  test.describe('Unfollow from User Cards', () => {
    ...
    test('should unfollow from feed session cards', async ({ page }) => {
      ...
      const followButton = sessionCard
        .getByRole('button', { name: /^Follow$/i })
        .first();
      ...
      // Unfollow
      const followingButton = sessionCard
        .getByRole('button', { name: /Following/i })
        .first();
      await followingButton.click();
      await page.waitForTimeout(500);
      ...
      // Verify
      const followButtonAfter = sessionCard
        .getByRole('button', { name: /^Follow$/i })
        .first();
      await expect(followButtonAfter).toBeVisible();
    });
  ...
  test.describe('Unfollow from Multiple Entry Points', () => {
    test('should maintain consistent state across multiple components', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
      ...
      const sidebarFollowButton = page
        .getByRole('button', { name: /^Follow$/i })
        .first();
  ...
  test.describe('Error Handling', () => {
    test('should handle already unfollowed state gracefully', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
      ...
      const followingButtonAgain = page
        .getByRole('button', { name: /Following/i })
        .first();
    ...
    test('should handle network errors gracefully', async ({ page }) => {
      ...
      const hasErrorMessage = await page
        ...
        .count();
      const hasFollowingButton = await page
        .getByRole('button', { name: /Following/i })
        .count();
  ...
  test.describe('UI Feedback and Loading States', () => {
    test('should disable button during unfollow operation', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
      ...
      // Get initial state
      const isInitiallyDisabled = await followingButton
        .isDisabled()
        .catch(() => false);
      ...
      const isDisabledDuringOp = await followingButton
        .isDisabled()
        .catch(() => false);
      ...
      expect(
        isInitiallyDisabled === false || isDisabledDuringOp === true
      ).toBeTruthy();
    });

    test('should update button text from Following to Follow', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
  ...
  test.describe('Accessibility', () => {
    test('should have accessible follow/unfollow buttons', async ({ page }) => {
      ...
      const isFocused = await followButton.evaluate(
    ...
    test('should have proper ARIA labels on follow/unfollow buttons', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
      ...
      expect(
        ariaLabel?.toLowerCase().includes('follow') ||
          textContent?.toLowerCase().includes('follow')
      ).toBeTruthy();
      ...
      const followingButton = page
        .getByRole('button', { name: /Following/i })
        .first();
      const followingAriaLabel =
        await followingButton.getAttribute('aria-label');
      const followingText = await followingButton.textContent();
  ...
  test.describe('Profile Page Integration', () => {
    test('should show correct follower count after unfollow', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
    ...
    test('should update follower count in real-time on profile', async ({
      page,
    }) => {
      await page.goto('/');
      await waitForNetworkIdle(page);
