...
test.describe('Activity Management Journey', () => {
  ...
  test.describe('Create Custom Activity', () => {
    ...
    test('should create activity with all fields', async ({ page }) => {
      ...
      if ((await createButton.count()) > 0) {
        ...
        const nameInput = page
          .locator(
            'input[placeholder*="Name"], input[placeholder*="Activity"], input[type="text"]'
          )
          .first();
        ...
        if ((await nameInput.count()) > 0) {
          ...
          const colorInputs = page
            .locator('input[type="color"], button[class*="color"]')
            .first();
  ...
  test.describe('Edit Custom Activity', () => {
    test('should find and open edit modal for existing activity', async ({
      page,
    }) => {
      await page.goto('/settings/activities');
      await page.waitForLoadState('networkidle');
    ...
    test('should update activity name', async ({ page }) => {
      ...
      const editButtons = page.locator(
        'button:has-text("Edit"), button:has-text("Pencil")'
      );
      ...
      if ((await editButtons.count()) > 0) {
        ...
        if ((await nameInput.count()) > 0) {
          ...
          if ((await submitButton.count()) > 0) {
            ...
            expect(
              updatedText?.includes(newName) || updatedText?.includes('updated')
            ).toBeTruthy();
          }
        }
      }
  ...
  test.describe('Delete Custom Activity', () => {
    ...
    test('should cancel delete action', async ({ page }) => {
      ...
      const deleteButtons = page.locator(
        'button:has-text("Delete"), button:has-text("Trash")'
      );

      if ((await deleteButtons.count()) > 0) {
        const countBefore = await page
          .locator('[class*="activity"], [class*="Activity"]')
          .count();
    ...
    test('should confirm delete activity', async ({ page }) => {
      ...
      if ((await deleteButtons.count()) > 0) {
        ...
        const confirmButton = page
          .locator(
            'button:has-text("Delete"), button:has-text("Yes"), button:has-text("Remove")'
          )
          .first();
  ...
  test.describe('Activity Limit Validation', () => {
    ...
    test('should disable create button when at limit', async ({ page }) => {
      ...
      if ((await createButton.count()) > 0) {
        ...
        const isDisabled = await createButton.evaluate(
          (btn: HTMLButtonElement) => btn.hasAttribute('disabled')
        );
  ...
  test.describe('Accessibility', () => {
    ...
    test('should have keyboard navigation through activities', async ({
      page,
    }) => {
      await page.goto('/settings/activities');
      await page.waitForLoadState('networkidle');
    ...
    test('should support screen reader navigation', async ({ page }) => {
      ...
      const elementsWithAriaLabel = page.locator(
