...
describe('Activity Preferences API', () => {
  ...
  describe('updateActivityPreference()', () => {
    it('should require userId and typeId', async () => {
      // Act & Assert
      await expect(
        updateActivityPreference('', 'activity-1', 1, new Date())
      ).rejects.toThrow();
      await expect(
        updateActivityPreference('user-123', '', 1, new Date())
      ).rejects.toThrow();
    });
    ...
    it('should update preference with new usage data', async () => {
      ...
      // Act
      const result = await updateActivityPreference(
        userId,
        typeId,
        useCount,
        lastUsed
    ...
    it('should increment useCount on repeated activity usage', async () => {
      ...
      // Act - First use
      const firstUpdate = await updateActivityPreference(
        userId,
        typeId,
        1,
        new Date()
    ...
    it('should update lastUsed timestamp on each activity use', async () => {
      ...
      expect(result.lastUsed.toDate().getTime()).toBeLessThanOrEqual(
        Date.now()
      );
    });
    ...
    it('should preserve useCount when updating lastUsed', async () => {
      ...
      // Act
      const initial = await updateActivityPreference(
        userId,
        typeId,
        useCount,
        oldTime
      );
      const updated = await updateActivityPreference(
        userId,
        typeId,
        useCount,
        new Date()
  ...
  describe('getRecentActivities()', () => {
    ...
    it('should validate limit parameter', async () => {
      ...
      await expect(getRecentActivities('user-123', 21)).rejects.toThrow(
        /Limit must be between/
      );

      // Act & Assert - limit too low
      await expect(getRecentActivities('user-123', 0)).rejects.toThrow(
        /Limit must be between/
      );

      // Act & Assert - negative limit
      await expect(getRecentActivities('user-123', -1)).rejects.toThrow(
        /Limit must be between/
      );
    });
  ...
  describe('Activity Preference Tracking', () => {
    ...
    it('should maintain separate preferences for different activities', async () => {
      // Act
      const work = await updateActivityPreference(
        'user-123',
        'work',
        10,
        new Date()
      );
      const coding = await updateActivityPreference(
        'user-123',
        'coding',
        8,
        new Date()
      );
      const reading = await updateActivityPreference(
        'user-123',
        'reading',
        3,
        new Date()
    ...
    it('should maintain separate preferences for different users', async () => {
      // Act
      const user1Activity = await updateActivityPreference(
        'user-1',
        'work',
        15,
        new Date()
      );
      const user2Activity = await updateActivityPreference(
        'user-2',
        'work',
        5,
        new Date()
  ...
  describe('Recent Activities Sorting', () => {
    ...
    it('should break ties by useCount when lastUsed is same', async () => {
      ...
      const activity1 = await updateActivityPreference(
        'user-123',
        'activity1',
        10,
        sameTime
      );
      const activity2 = await updateActivityPreference(
        'user-123',
        'activity2',
        5,
        sameTime
