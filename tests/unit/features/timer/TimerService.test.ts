...
describe('TimerService', () => {
  ...
  beforeEach(() => {
    ...
    ;(
      ActiveSessionRepository as jest.MockedClass<typeof ActiveSessionRepository>
  ...
  describe('startTimer', () => {
    it('should start a new timer successfully', async () => {
      ...
      mockActiveSessionRepoInstance.saveActiveSession.mockResolvedValue(undefined)
    ...
    it('should throw error if timer already exists', async () => {
      ...
      mockActiveSessionRepoInstance.getActiveSession.mockResolvedValue(existingSession)
  ...
  describe('pauseTimer', () => {
    it('should pause an active timer', async () => {
      ...
      mockActiveSessionRepoInstance.getActiveSession.mockResolvedValue(activeSession)
      mockActiveSessionRepoInstance.saveActiveSession.mockResolvedValue(undefined)
    ...
    it('should throw error if no active timer', async () => {
      ...
      // ACT & ASSERT
      await expect(timerService.pauseTimer('user-123')).rejects.toThrow('No active timer to pause')
    })
    ...
    it('should throw error if timer already paused', async () => {
      ...
      mockActiveSessionRepoInstance.getActiveSession.mockResolvedValue(pausedSession)

      // ACT & ASSERT
      await expect(timerService.pauseTimer('user-123')).rejects.toThrow('Timer is already paused')
    })
  })
  ...
  describe('resumeTimer', () => {
    ...
    it('should throw error if timer already running', async () => {
      ...
      mockActiveSessionRepoInstance.getActiveSession.mockResolvedValue(runningSession)

      // ACT & ASSERT
      await expect(timerService.resumeTimer('user-123')).rejects.toThrow('Timer is already running')
    })
  })
  ...
  describe('completeTimer', () => {
    it('should complete timer and save as session', async () => {
      ...
      mockActiveSessionRepoInstance.getActiveSession.mockResolvedValue(activeSession)
      mockActiveSessionRepoInstance.deleteActiveSession.mockResolvedValue(undefined)
  ...
  describe('stopTimer', () => {
    it('should stop and discard timer', async () => {
      ...
      // ASSERT
      expect(mockActiveSessionRepoInstance.deleteActiveSession).toHaveBeenCalledWith(
        'user-123',
        'session-789'
      )
    })
    ...
    it('should throw error if no active timer to stop', async () => {
      ...
      // ACT & ASSERT
      await expect(timerService.stopTimer('user-123')).rejects.toThrow('No active timer to stop')
    })
  })
  ...
  describe('updateTimerMetadata', () => {
    ...
    it('should throw error if no active timer', async () => {
      ...
      // ACT & ASSERT
      await expect(timerService.updateTimerMetadata('user-123', 'Title')).rejects.toThrow(
        'No active timer to update'
      )
    })
  })
  ...
  describe('adjustStartTime', () => {
    ...
    it('should throw error if start time is in future', async () => {
      ...
      mockActiveSessionRepoInstance.getActiveSession.mockResolvedValue(
        new ActiveSession('session-789', 'user-123', 'project-456', new Date(), 'running', 0)
      )

      // ACT & ASSERT
      await expect(timerService.adjustStartTime('user-123', futureTime)).rejects.toThrow(
    ...
    it('should throw error if no active timer', async () => {
      ...
      // ACT & ASSERT
      await expect(timerService.adjustStartTime('user-123', new Date())).rejects.toThrow(
        'No active timer to adjust'
      )
    })
  })
  ...
  describe('autoSaveSession', () => {
    it('should auto-save active session', async () => {
      ...
      // ASSERT
      expect(mockActiveSessionRepoInstance.saveActiveSession).toHaveBeenCalledWith(activeSession)
    })
    ...
    it('should do nothing if no active session', async () => {
      ...
      // ASSERT
      expect(mockActiveSessionRepoInstance.saveActiveSession).not.toHaveBeenCalled()
    })
  })
})
