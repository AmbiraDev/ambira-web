...
describe('StreakService', () => {
  ...
  describe('getStreakData', () => {
    it('should get streak data for user', async () => {
      // ARRANGE
      ;(firebaseApi.streak.getStreakData as jest.Mock).mockResolvedValue(mockStreakData)
    ...
    it('should handle API errors gracefully', async () => {
      // ARRANGE
      ;(firebaseApi.streak.getStreakData as jest.Mock).mockRejectedValue(new Error('API Error'))
  ...
  describe('getStreakStats', () => {
    it('should get streak statistics', async () => {
      // ARRANGE
      ;(firebaseApi.streak.getStreakStats as jest.Mock).mockResolvedValue(mockStreakStats)
    ...
    it('should handle API errors gracefully', async () => {
      // ARRANGE
      ;(firebaseApi.streak.getStreakStats as jest.Mock).mockRejectedValue(new Error('API Error'))
  ...
  describe('updateStreakVisibility', () => {
    it('should update streak to public', async () => {
      // ARRANGE
      ;(firebaseApi.streak.updateStreakVisibility as jest.Mock).mockResolvedValue(undefined)

      // ACT
      await streakService.updateStreakVisibility('user-1', true)

      // ASSERT
      expect(firebaseApi.streak.updateStreakVisibility).toHaveBeenCalledWith('user-1', true)
    })
    ...
    it('should update streak to private', async () => {
      ...
      // ASSERT
      expect(firebaseApi.streak.updateStreakVisibility).toHaveBeenCalledWith('user-1', false)
    })

    it('should propagate API errors', async () => {
      // ARRANGE
      ;(firebaseApi.streak.updateStreakVisibility as jest.Mock).mockRejectedValue(
        new Error('Update failed')
      )

      // ACT & ASSERT
      await expect(streakService.updateStreakVisibility('user-1', true)).rejects.toThrow(
        'Update failed'
      )
    })
  })
