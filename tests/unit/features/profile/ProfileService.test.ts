...
const baseUser = new User('profile-1', 'test', 'Test User', 'test@example.com', new Date())
...
describe('ProfileService', () => {
  ...
  it('enforces follower-only privacy checks', async () => {
    ...
    await expect(service.canViewProfile(profileUser, 'viewer')).resolves.toBe(true)

    isFollowing.mockResolvedValueOnce(false)
    await expect(service.canViewProfile(profileUser, 'viewer')).resolves.toBe(false)
  })
  ...
  it('propagates errors from repository follow/unfollow operations', async () => {
    ...
    await expect(service.followUser('same', 'same')).rejects.toThrow('Cannot follow yourself')
