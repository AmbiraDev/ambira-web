...
describe('CreateCustomActivityModal', () => {
  ...
  describe('Rendering', () => {
    it('should not render when isOpen is false', () => {
      ...
      // Assert
      expect(
        screen.queryByText('Create Custom Activity')
  ...
  describe('Form Validation', () => {
    ...
    it('should require icon selection', async () => {
      ...
      expect(
        screen.queryByText('Please select an icon')
      ).not.toBeInTheDocument();
    });
  ...
  describe('Form Submission', () => {
    it('should submit valid form data', async () => {
      // Arrange
      const user = userEvent.setup();
      const { useCreateCustomActivity } = jest.requireMock(
        '@/hooks/useActivityTypes'
      );
      const mockMutateAsync = jest
        .fn()
        .mockResolvedValue({ id: 'new-activity' });
  ...
  describe('Preview', () => {
    ...
    it('should not show description in preview when empty', () => {
      ...
      const inPreview = descElements.filter(el =>
        el.closest('[class*="bg-gray-50"]')
      );
  ...
  describe('Modal Lifecycle', () => {
    it('should reset form when modal opens', async () => {
      ...
      const freshNameInput = screen.getByLabelText(
        /Activity Name/
      ) as HTMLInputElement;
