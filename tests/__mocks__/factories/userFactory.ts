...
export function createMockUserBatch(count: number, baseOptions: CreateUserOptions = {}): User[] {
  return Array.from({ length: count }, (_, i) =>
    createMockUser({
      ...baseOptions,
      id: `user-batch-${i}`,
      username: baseOptions.username ? `${baseOptions.username}${i}` : `user${i}`,
    })
  )
}
 ...
 */
export function createMockFollowersOnlyUser(options: CreateUserOptions = {}): User {
  return createMockUser({
    ...options,
