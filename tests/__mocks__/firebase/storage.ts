...
export const mockStorage = {
  ...
  uploadBytes: jest
    .fn()
    .mockImplementation(async (ref: MockStorageReference, data: Blob, _metadata?: unknown) => {
      mockStorageData.set(ref.fullPath, data)
      return createMockUploadResult(ref)
    }),

  uploadBytesResumable: jest.fn().mockImplementation((ref: MockStorageReference, data: Blob) => {
    // Mock upload task
    const task = {
      on: jest.fn(
        (
  ...
  uploadString: jest
    .fn()
    .mockImplementation(
      async (ref: MockStorageReference, data: string, _format?: string, _metadata?: unknown) => {
  ...
  getDownloadURL: jest.fn().mockImplementation(async (ref: MockStorageReference) => {
  ...
  deleteObject: jest.fn().mockImplementation(async (ref: MockStorageReference) => {
  ...
  listAll: jest.fn().mockImplementation(async (ref: MockStorageReference) => {
    ...
    mockStorageData.forEach((_, path) => {
      if (path.startsWith(ref.fullPath)) {
        const relativePath = path.substring(ref.fullPath.length).replace(/^\//, '')
  ...
  getMetadata: jest.fn().mockImplementation(async (ref: MockStorageReference) => {
    if (!mockStorageData.has(ref.fullPath)) {
      throw new Error('storage/object-not-found')
    }
  ...
  updateMetadata: jest
    .fn()
    .mockImplementation(async (ref: MockStorageReference, metadata: Record<string, unknown>) => {
      if (!mockStorageData.has(ref.fullPath)) {
        throw new Error('storage/object-not-found')
      }
