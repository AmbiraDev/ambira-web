...
describe('Integration: User Login Flow', () => {
  ...
  beforeEach(() => {
    ...
    mockFirebaseApi.auth.signIn.mockImplementation(async (email: string, _password: string) => {
      const user = Array.from(testFirebaseStore['users'].values()).find((u) => u.email === email)
  ...
  it('completes full login flow: credentials → API → auth state → redirect', async () => {
    ...
    const loggedInUser = await mockFirebaseApi.auth.signIn('test@example.com', 'password123')

    // Assert: Login successful
    expect(mockFirebaseApi.auth.signIn).toHaveBeenCalledWith('test@example.com', 'password123')
  ...
  it('rejects login with invalid email', async () => {
    ...
    await expect(mockFirebaseApi.auth.signIn('wrong@example.com', 'password123')).rejects.toThrow(
      'Invalid credentials'
    )
  })
  ...
  it('rejects login with invalid password', async () => {
    ...
    mockFirebaseApi.auth.signIn.mockImplementationOnce(async (email: string, password: string) => {
      const user = Array.from(testFirebaseStore['users'].values()).find((u) => u.email === email)
      if (!user) throw new Error('Invalid credentials')
    ...
    await expect(mockFirebaseApi.auth.signIn('test@example.com', 'wrongpassword')).rejects.toThrow(
      'Invalid credentials'
    )
  })
  ...
  it('handles network errors during login', async () => {
    // Mock network error
    mockFirebaseApi.auth.signIn.mockRejectedValueOnce(new Error('Network error'))

    // Act & Assert: Handle error
    await expect(mockFirebaseApi.auth.signIn('test@example.com', 'password123')).rejects.toThrow(
      'Network error'
    )
  })
  ...
  it('handles rate limiting on login attempts', async () => {
    ...
    let attemptCount = 0
    mockFirebaseApi.auth.signIn.mockImplementation(async (_email: string, _password: string) => {
      attemptCount++
      if (attemptCount > 3) {
    ...
    await expect(mockFirebaseApi.auth.signIn('test@example.com', 'wrong1')).rejects.toThrow(
      'Invalid credentials'
    )

    await expect(mockFirebaseApi.auth.signIn('test@example.com', 'wrong2')).rejects.toThrow(
      'Invalid credentials'
    )

    await expect(mockFirebaseApi.auth.signIn('test@example.com', 'wrong3')).rejects.toThrow(
      'Invalid credentials'
    )

    await expect(mockFirebaseApi.auth.signIn('test@example.com', 'wrong4')).rejects.toThrow(
  ...
  it('case-insensitive email login', async () => {
    ...
    mockFirebaseApi.auth.signIn.mockImplementationOnce(async (email: string, _password: string) => {
  ...
  it('clears previous session data on new login', async () => {
    ...
    // Act: Login as user1
    const loggedInUser1 = await mockFirebaseApi.auth.signIn('user1@test.com', 'password123')
    expect(loggedInUser1.id).toBe(user1.id)

    // Act: Logout and login as user2
    await mockFirebaseApi.auth.signOut()

    const loggedInUser2 = await mockFirebaseApi.auth.signIn('user2@test.com', 'password123')
