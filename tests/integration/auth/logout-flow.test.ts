...
describe('Integration: User Logout Flow', () => {
  ...
  it('completes full logout flow: logout → clear auth → clear cache → redirect', async () => {
    ...
    const cachedUser = queryClient.getQueryData(CACHE_KEYS.USER_PROFILE(user.id))
  ...
  it('clears user-specific cache on logout', async () => {
    ...
    expect(queryClient.getQueryData(CACHE_KEYS.USER_PROFILE(user.id))).toBeUndefined()
    expect(queryClient.getQueryData(CACHE_KEYS.PROJECTS(user.id))).toBeUndefined()
    expect(queryClient.getQueryData(CACHE_KEYS.SESSIONS(user.id))).toBeUndefined()
  })
  ...
  it('handles logout errors gracefully', async () => {
    ...
    // Mock logout error
    mockFirebaseApi.auth.signOut.mockRejectedValueOnce(new Error('Logout failed'))

    // Act & Assert: Handle error
    await expect(mockFirebaseApi.auth.signOut()).rejects.toThrow('Logout failed')
  })
  ...
  it('allows immediate re-login after logout', async () => {
    ...
    const reloggedUser = await mockFirebaseApi.auth.signIn('test@example.com', 'password123')
