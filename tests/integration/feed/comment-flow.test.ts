...
jest.mock('@/lib/api', () => ({
  firebaseCommentsApi: {
    ...
    getComments: jest.fn(async (sessionId: string) => testFirebaseStore.getComments(sessionId)),
...
describe('Integration: Session Comment Flow', () => {
  ...
  it('adds comment with optimistic update and API call', async () => {
    ...
    const comment = await mockFirebaseApi.social.comment(session.id, commentData)

    // Assert: Comment created
    expect(mockFirebaseApi.social.comment).toHaveBeenCalledWith(session.id, commentData)
  ...
  it('prevents empty comments', async () => {
    // Mock validation
    mockFirebaseApi.social.comment.mockImplementationOnce(async (sessionId: string, data: any) => {
