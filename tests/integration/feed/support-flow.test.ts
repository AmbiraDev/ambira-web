...
describe('Integration: Session Support Flow', () => {
  ...
  it('supports session with optimistic update and API call', async () => {
    ...
    // Assert: API called
    expect(mockFirebaseApi.social.support).toHaveBeenCalledWith(
      session.id,
      user.id
    );
  ...
  it('unsupports session and reverts count', async () => {
    ...
    // Assert: API called
    expect(mockFirebaseApi.social.unsupport).toHaveBeenCalledWith(
      session.id,
      user.id
    );
  ...
  it('prevents duplicate support from same user', async () => {
    ...
    await expect(
      mockFirebaseApi.social.support(session.id, user.id)
  ...
  it('rolls back optimistic update on API error', async () => {
    ...
    // Mock API error
    mockFirebaseApi.social.support.mockRejectedValueOnce(
