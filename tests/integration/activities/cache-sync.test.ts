...
describe('Integration: Cache Synchronization for Activities', () => {
  ...
  it('updates custom activity cache when activity created', async () => {
    ...
    const initialCustoms = await mockFirebaseApi.activityTypes.getUserCustom(
      user.id
    );
  ...
  it('updates activity preferences cache when session created', async () => {
    ...
    const initialPrefs = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
      5
    );
    queryClient.setQueryData(
      ACTIVITY_PREFERENCE_KEYS.recent(user.id, 5),
      initialPrefs
    );
    ...
    const updatedPrefs = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
      5
    );
  ...
  it('updates all preference caches when activity used', async () => {
    ...
    queryClient.setQueryData(
      ACTIVITY_PREFERENCE_KEYS.detail(user.id, 'work'),
      null
    );
    ...
    const recentData = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
      5
    );
    ...
    const cachedList = queryClient.getQueryData(
      ACTIVITY_PREFERENCE_KEYS.list(user.id)
    );
  ...
  it('maintains consistency across activity type and preference caches', async () => {
    ...
    const prefs = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
      5
    );
    ...
    // Update both caches
    const updatedActivities = await mockFirebaseApi.activityTypes.getAll(
      user.id
    );
    const updatedPrefs = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
    ...
    const cachedActivities = queryClient.getQueryData(
      ACTIVITY_TYPE_KEYS.combined(user.id)
    );
    ...
    for (const pref of cachedPrefs) {
      const activity = cachedActivities.find(
        (a: ActivityType) => a.id === pref.typeId
      );
    ...
    queryClient.setQueryData(
      ...
      (old: ActivityType[] | undefined) => {
        ...
        return old.map((a: ActivityType) =>
          a.id.startsWith('temp-') ? serverActivity : a
        );
      }
    );
  ...
  it('rolls back optimistic updates on error', async () => {
    // Arrange: Initial cache state
    const initialActivity = await mockFirebaseApi.activityTypes.create(
      user.id,
      {
        name: 'Initial',
        icon: 'Test',
  ...
  it('invalidates cache after server mutation completes', async () => {
    ...
    const isStale = queryClient.getQueryState(
      ACTIVITY_TYPE_KEYS.custom(user.id)
    )?.isInvalidated;

    // Re-fetch fresh data
    const freshData = await mockFirebaseApi.activityTypes.getUserCustom(
      user.id
    );
