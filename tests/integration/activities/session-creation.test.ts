...
jest.mock('@/lib/api', () => ({
  firebaseSessionApi: {
    ...
    getActiveSession: (userId: string) =>
      mockFirebaseApi.activeSession.get(userId),
    clearActiveSession: (userId: string) =>
      mockFirebaseApi.activeSession.clear(userId),
...
describe('Integration: Session Creation with Activities', () => {
  ...
  it('updates activity preference after session creation', async () => {
    ...
    const pref = testFirebaseStore.getActivityPreference(
      user.id,
      session.activityId
    );
  ...
  it('optimistically updates activity preference cache', async () => {
    ...
    queryClient.setQueryData(
      PREFERENCE_KEY,
      (old: UserActivityPreference[] | undefined) => [newPref, ...(old || [])]
    );
