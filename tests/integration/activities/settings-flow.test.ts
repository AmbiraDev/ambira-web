...
describe('Integration: Activity Settings Flow', () => {
  ...
  it('creates activity from settings page with form', async () => {
    ...
    const created = await mockFirebaseApi.activityTypes.create(user.id, formData)
  ...
  it('displays validation errors inline in form', async () => {
    ...
    await expect(mockFirebaseApi.activityTypes.create(user.id, invalidData)).rejects.toThrow()
  })
  ...
  it('optimistically removes from cache on delete', async () => {
    ...
    const remaining = await mockFirebaseApi.activityTypes.getUserCustom(user.id)
  ...
  it('updates combined activity types cache on settings changes', async () => {
    ...
    const cached = queryClient.getQueryData(['activityTypes', 'combined', user.id])
