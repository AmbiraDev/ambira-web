...
jest.mock('@/lib/api/activityPreferences', () => ({
  ...
  getAllActivityPreferences: (userId: string) =>
    mockFirebaseApi.activityPreferences.getAll(userId),
}));
...
describe('Integration: Activity Picker Flow', () => {
  ...
  it('loads picker with system activities available', async () => {
    ...
    const workActivity = allActivities.find(
      (a: ActivityType) => a.id === 'work'
    );
    const codingActivity = allActivities.find(
      (a: ActivityType) => a.id === 'coding'
    );
  ...
  it('shows recent activities when picker loads', async () => {
    ...
    const recentActivities =
      await mockFirebaseApi.activityPreferences.getRecent(user.id, 5);
    ...
    const activityIds = recentActivities.map(
      (p: UserActivityPreference) => p.typeId
    );
  ...
  it('shows vertical list of all activities in picker', async () => {
    ...
    const guitarActivity = allActivities.find(
      (a: ActivityType) => a.id === custom1.id
    );
    const readingActivity = allActivities.find(
      (a: ActivityType) => a.id === custom2.id
    );
  ...
  it('selects activity and updates cache optimistically', async () => {
    ...
    queryClient.setQueryData(
      ['activityTypes', 'combined', user.id],
      allActivities
    );
    ...
    const recentActivities =
      await mockFirebaseApi.activityPreferences.getRecent(user.id, 5);
    queryClient.setQueryData(
      ['activityPreferences', 'recent', user.id],
      recentActivities
    );

    // Assert: Cache updated with selected activity
    const cachedRecent = queryClient.getQueryData([
      'activityPreferences',
      'recent',
      user.id,
    ]);
  ...
  it('creates new activity from picker and immediately available', async () => {
    ...
    const createdActivity = allActivities.find(
      (a: ActivityType) => a.id === newActivity.id
    );
  ...
  it('recent activities update after selecting activity', async () => {
    ...
    let recentActivities = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
      5
    );
    ...
    recentActivities = await mockFirebaseApi.activityPreferences.getRecent(
      user.id,
      5
    );
    expect(recentActivities.length).toBeGreaterThan(0);
  ...
  it('picker respects custom activity custom properties', async () => {
    ...
    const allActivities = await mockFirebaseApi.activityTypes.getAll(user.id);
    const pickerActivity = allActivities.find(
      (a: ActivityType) => a.id === custom.id
    );
    ...
    expect(pickerActivity?.description).toBe(
      'Advanced meditation and mindfulness practice'
    );
  });
  ...
  it('handles rapid activity selections without conflicts', async () => {
    ...
    const recentIds = recentActivities.map(
      (p: UserActivityPreference) => p.typeId
    );
    for (const activityId of activities) {
  ...
  it('supports custom activity descriptions in picker', async () => {
    ...
    const guitarActivity = allActivities.find(
      (a: ActivityType) => a.id === guitar.id
    );
    const pianoActivity = allActivities.find(
      (a: ActivityType) => a.id === piano.id
    );

    expect(guitarActivity?.description).toBe(
      'Acoustic and electric guitar practice'
    );
    expect(pianoActivity?.description).toBe(
      'Classical and modern piano practice'
    );
  });
});
