...
describe('Integration: Activity Lifecycle Flow', () => {
  ...
  it('creates custom activity and appears in picker', async () => {
    ...
    const guitarActivity = allActivities.find(
      (a: ActivityType) =>
        a.id === newActivity.id && a.name === 'Guitar Practice'
    );
  ...
  it('edits custom activity and updates all references', async () => {
    ...
    const updated_activity = customActivities.find(
      (a: ActivityType) => a.id === created.id
    );
  ...
  it('deletes custom activity and removes from preferences', async () => {
    ...
    const deletedActivity = customActivities.find(
      (a: ActivityType) => a.id === created.id
    );
  ...
  it('maintains correct order of system defaults and custom activities', async () => {
    ...
    const systemActivities = allActivities.filter(
      (a: ActivityType) => a.isSystem
    );
    const customActivities = allActivities.filter(
      (a: ActivityType) => !a.isSystem
    );
    ...
    const minSystemOrder = Math.min(
      ...systemActivities.map((a: ActivityType) => a.order)
    );
    const maxSystemOrder = Math.max(
      ...systemActivities.map((a: ActivityType) => a.order)
    );
    const minCustomOrder = Math.min(
      ...customActivities.map((a: ActivityType) => a.order)
    );
  ...
  it('cannot edit or delete system activity types', async () => {
    ...
    await expect(
      mockFirebaseApi.activityTypes.delete('work', user.id)
    ).rejects.toThrow('Cannot delete default activity types');
  });
  ...
  it('supports activity categories for organization', async () => {
    ...
    const foundProductivity = allActivities.find(
      (a: ActivityType) => a.id === productivity.id
    );
    const foundLearning = allActivities.find(
      (a: ActivityType) => a.id === learning.id
    );
    const foundCreative = allActivities.find(
      (a: ActivityType) => a.id === creative.id
    );
