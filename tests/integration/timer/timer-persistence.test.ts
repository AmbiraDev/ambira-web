...
describe('Integration: Timer Persistence', () => {
  ...
  it('clears stale sessions older than 24 hours', async () => {
    ...
    mockFirebaseApi.activeSession.get.mockImplementationOnce(async (userId: string) => {
  ...
  it('preserves timer through network interruption', async () => {
    ...
    mockFirebaseApi.activeSession.get.mockRejectedValueOnce(new Error('Network error'))

    // Act: Attempt to fetch (fails)
    await expect(mockFirebaseApi.activeSession.get(user.id)).rejects.toThrow('Network error')
  ...
  it('updates cache on timer state changes', async () => {
    ...
    const cachedTimer = queryClient.getQueryData(CACHE_KEYS.ACTIVE_SESSION(user.id))
