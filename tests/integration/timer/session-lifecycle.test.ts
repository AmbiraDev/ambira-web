...
describe('Integration: Timer Session Lifecycle', () => {
  ...
  it('starts timer and persists to Firebase', async () => {
    ...
    expect(mockFirebaseApi.activeSession.save).toHaveBeenCalledWith(user.id, timerData)
  ...
  it('updates cache after completing session', async () => {
    ...
    const cachedActiveSession = queryClient.getQueryData(CACHE_KEYS.ACTIVE_SESSION(user.id))
  ...
  it('validates session duration when completing', async () => {
    ...
    // Mock validation
    mockFirebaseApi.sessions.create.mockImplementationOnce(async (data: any) => {
      if (data.duration < 60) {
        // Minimum 1 minute
