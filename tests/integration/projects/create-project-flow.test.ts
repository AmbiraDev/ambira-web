...
describe('Integration: Create Project Flow', () => {
  ...
  it('validates required fields', async () => {
    // Mock validation
    mockFirebaseApi.projects.create.mockImplementationOnce(
      async (data: any) => {
    ...
    await expect(
      mockFirebaseApi.projects.create({ userId: user.id } as any)
    ).rejects.toThrow('Project name required');

    // Act & Assert: Missing userId
    await expect(
      mockFirebaseApi.projects.create({ name: 'Test' } as any)
