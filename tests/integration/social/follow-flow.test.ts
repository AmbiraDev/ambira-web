...
describe('Integration: Follow/Unfollow Flow', () => {
  ...
  it('follows user and updates follower/following counts', async () => {
    ...
    expect(mockFirebaseApi.social.follow).toHaveBeenCalledWith(user.id, targetUser.id)
  ...
  it('unfollows user and decrements counts', async () => {
    ...
    expect(mockFirebaseApi.social.unfollow).toHaveBeenCalledWith(user.id, targetUser.id)
  ...
  it('prevents duplicate follow', async () => {
    ...
    await expect(mockFirebaseApi.social.follow(user.id, targetUser.id)).rejects.toThrow(
      'Already following'
    )
  ...
  it('updates cache after follow/unfollow', async () => {
    ...
    queryClient.setQueryData(CACHE_KEYS.USER_PROFILE(targetUser.id), targetUser)
    ...
    queryClient.setQueryData(CACHE_KEYS.USER_PROFILE(targetUser.id), updatedFollowing)

    // Assert: Cache reflects follow
    const cachedFollower = queryClient.getQueryData(CACHE_KEYS.USER_PROFILE(user.id))
    const cachedFollowing = queryClient.getQueryData(CACHE_KEYS.USER_PROFILE(targetUser.id))
  ...
  it('prevents self-follow', async () => {
    ...
    await expect(mockFirebaseApi.social.follow(user.id, user.id)).rejects.toThrow(
